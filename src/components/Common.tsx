...
import { useAuth, MembershipTier as AuthMembershipTier } from '../context/AuthContext';
...
export const AppHeader = () => {
  ...
  useEffect(() => {
    const handleOpenModal = () => setIsMembershipModalOpen(true);
    ...
    window.addEventListener('open-membership-modal', handleOpenModal);
    ...
    return () => {
      window.removeEventListener('open-membership-modal', handleOpenModal);
      ...
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-700/30 px-6 md:px-10 py-4 bg-background-dark/80 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden md:flex items-center gap-2">
            ...
            <NavLink to="/add-concept" label="Agregar" />
            <NavLink to="/forum" label="Foro" />
            <NavLink to="/news" label="Noticias" />
            <NavLink to="/marketplace" label="Mercado" />
            <NavLink to="/wallet" label="Mi Cartera" />
          </nav>
        </div>
            ...
            {isMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-[60] animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="p-2 flex flex-col gap-1">
                  ...
                  <button 
                    onClick={() => { setIsMembershipModalOpen(true); setIsMenuOpen(false); }}
                  ...
                  >
                    ...
                    <span>Mercado BIM</span>
                  </button>
                  <button 
                    onClick={() => { navigate('/register-company'); setIsMenuOpen(false); }}
                    className="flex items-center gap-3 px-4 py-3 text-sm text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors text-left"
                  >
                  ...
                  <button 
                    onClick={() => { navigate('/providers'); setIsMenuOpen(false); }}
                    className="flex items-center gap-3 px-4 py-3 text-sm text-slate-300 hover:bg-primary/20 hover:text-white rounded-lg transition-colors text-left font-bold"
                  >
                    <Building2 size={16} className="text-primary" />
                    <span>Registrar mi Empresa</span>
                  </button>
        ...
        {isMobileNavOpen && (
          ...
          >
            <div className="p-4 flex flex-col gap-2">
              ...
              <MobileNavLink to="/add-concept" label="Agregar" icon={<Plus size={16} />} onClick={() => setIsMobileNavOpen(false)} />
              <MobileNavLink to="/forum" label="Foro" icon={<MessageSquare size={16} />} onClick={() => setIsMobileNavOpen(false)} />
              <MobileNavLink to="/news" label="Noticias" icon={<FileText size={16} />} onClick={() => setIsMobileNavOpen(false)} />
              <MobileNavLink to="/marketplace" label="Mercado" icon={<ShoppingBag size={16} />} onClick={() => setIsMobileNavOpen(false)} />
              <MobileNavLink to="/wallet" label="Mi Cartera" icon={<Wallet size={16} />} onClick={() => setIsMobileNavOpen(false)} />
              
              <div className="h-px bg-slate-800 my-2"></div>
              
              <button 
                onClick={() => { setIsMembershipModalOpen(true); setIsMobileNavOpen(false); }}
