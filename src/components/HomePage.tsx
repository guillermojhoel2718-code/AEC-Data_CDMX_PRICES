...
import { Search, ArrowRight, ShieldCheck, CheckCircle2, Zap, FileText, Building2, BarChart3, Coins, Lock, Crown } from 'lucide-react';
import { NavHeader } from './NavHeader';
import { motion } from 'motion/react';

const USER_TIERS = [
  {
    id: 'profesional', icon: <Crown className="w-6 h-6 text-[#6366F1]" />, label: 'Profesional',
    badge: 'Con API Key', color: 'border-[#6366F1]/40 bg-[#6366F1]/5', badgeColor: 'bg-[#6366F1]/20 text-[#A5B4FC]',
    ...
    cta: 'Explorar catálogo', route: '/explorer',
  },
  {
    id: 'no_profesional', icon: <Zap className="w-6 h-6 text-amber-400" />, label: 'No Profesional',
    badge: 'Público', color: 'border-amber-500/40 bg-amber-500/5', badgeColor: 'bg-amber-500/20 text-amber-300',
    ...
    cta: 'Buscar insumos', route: '/insumos',
  },
  {
    id: 'vendedor', icon: <Building2 className="w-6 h-6 text-cyan-400" />, label: 'Vendedor / Proveedor',
    badge: '$100 MXN/mes', color: 'border-cyan-500/40 bg-cyan-500/5', badgeColor: 'bg-cyan-500/20 text-cyan-300',
    ...
    cta: 'Próximamente', route: null, disabled: true,
  },
];
...
export const HomePage = () => {
  ...
  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); navigate(`/explorer?q=${encodeURIComponent(searchQuery)}`); };
        ...
        <div className="w-full max-w-[1100px] px-6 py-12 flex flex-col gap-16">
          ...
          <form onSubmit={handleSearch} className="w-full max-w-4xl mx-auto flex flex-col md:flex-row gap-4">
            <label className="relative flex grow h-16 group">
              <div className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors"><Search size={24} /></div>
            ...
            <button type="submit" className="bg-primary text-white px-10 h-16 rounded-2xl font-black uppercase tracking-widest hover:brightness-110 transition-all shadow-lg shadow-primary/20">Buscar</button>
          </form>
          ...
          <section className="space-y-6">
            <div className="text-center space-y-1">
              ...
              <p className="text-slate-500 text-sm">Todas las cuentas arrancan con 50 tokens de prueba</p>
            </div>
                  ...
                  <ul className="space-y-2.5 flex-1">
                    {tier.features.map((f, i) => (
                      <li key={i} className="flex items-start gap-2 text-xs text-slate-300">
                        <span className="mt-0.5 flex-shrink-0">{f.icon}</span>{f.text}
                      </li>
                    ))}
                  </ul>
                  {(tier as any).disabled
                    ? <div className="flex items-center gap-2 py-3 rounded-xl bg-slate-800/50 text-slate-500 text-xs font-bold justify-center border border-slate-700/50"><Lock className="w-3 h-3" />{tier.cta}</div>
                    : <Link to={tier.route!} className="py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2 border border-slate-700/50">{tier.cta}<ArrowRight className="w-3 h-3" /></Link>
                  }
                </div>
              ))}
            </div>
          ...
          <div className="rounded-2xl bg-gradient-to-br from-[#1A1A3E] to-[#0F0F2A] border border-[#6366F1]/20 p-8 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-[#6366F1]/20 flex items-center justify-center flex-shrink-0"><Coins className="w-7 h-7 text-[#6366F1]" /></div>
              <div>
                <h3 className="text-white font-black text-xl">Sistema de Tokens APUCMX</h3>
                <p className="text-slate-400 text-sm mt-1">$100 MXN = 100 tokens · Auditorías 10 tok · Generar APU 5 tok · Búsqueda gratis siempre</p>
              </div>
            </div>
            <div className="flex gap-3 flex-shrink-0">
              <Link to="/tokens" className="bg-[#6366F1] text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-[#4F51D9] transition-colors whitespace-nowrap shadow-lg shadow-[#6366F1]/20">Comprar Tokens</Link>
              <Link to="/insumos" className="bg-slate-800 text-white border border-slate-700 px-6 py-3 rounded-xl font-bold text-sm hover:bg-slate-700 transition-colors whitespace-nowrap">Ver Catálogo</Link>
            </div>
          </div>
          ...
          <section className="space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-white text-xl font-bold tracking-tight flex items-center gap-2"><ShieldCheck size={20} className="text-primary" />Precios de Referencia · CDMX 2026</h2>
              <Link to="/explorer" className="text-primary text-sm font-bold flex items-center gap-1 hover:underline">Ver catálogo <ArrowRight size={14} /></Link>
            </div>
          ...
          <div className="rounded-xl border border-slate-700/50 bg-slate-800/30 p-5 text-center">
            <p className="text-slate-400 text-xs">
              <span className="text-[#6366F1] font-bold">V2 (roadmap):</span> Token on-chain con OpenZeppelin + Arbitrum/Polygon. Validación comunitaria open-source. Carga de matrices (open source). Por ahora opera en Supabase + Stripe.
            </p>
          </div>
        </div>
      </main>
