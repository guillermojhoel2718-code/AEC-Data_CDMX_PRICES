...
export const ConceptProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchConcepts = async () => {
    ...
    setError(null);
    try {
      // Simula el tiempo de red para demostrar el "Loading State" elegante
      ...
      const { data: dbConcepts, error: dbError } = await supabase.from('concepts').select(`
        *,
        ...
        concept_labor (*),
        concept_equipment (*),
        concept_subcontracts (*),
        concept_overcost (*)
      `);
      
      if (dbError) throw dbError;
      
      const mappedConcepts = dbConcepts.map(row => mapDbConcept(
        row, 
        row.concept_materials || [], 
        row.concept_labor || [], 
        row.concept_equipment || [], 
        row.concept_subcontracts || [], 
        row.concept_overcost ? row.concept_overcost[0] : null
      ));
